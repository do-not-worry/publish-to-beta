...
function build()
{
    ...
    #urlencode
    jsonStr='{"parameter": {"name": "tag", "value": "'${tag}'"}, "statusCode": "303", "redirectTo": ".", "Jenkins-Crumb": "'${crumb}'"}'
    jsonStr=`echo ${jsonStr} | tr -d '\n' | od -An -tx1 | tr ' ' %`
    buildData="name=tag&value=${tag}&statusCode=303&redirectTo=.&Jenkins-Crumb=${crumb}&json=${jsonStr}&Submit=%E5%BC%80%E5%A7%8B%E6%9E%84%E5%BB%BA"
    buildRes=`curl -d "${buildData}" -A "${userAgent}" -b "${COOKIE_FILE}" "${buildUrl}"`
    if [[ ${buildRes} =~ "login" ]];then
        echo "登录信息已过期,重新登录中..."
        doLogin
